...
I believe the second code is wrong because it's in the wrong order.
...
After running the code I found that the longest movie was The Cure For Insomnia (5220 mins), and the oldest movie was the Blacksmith Scene (1893).
...
After observing this data, we can see that there were 1248 movies released in 1995.
...
```{r}
movies %>%
  filter(Comedy/Animation == 1 , votes >= 100) 
movies %>%
  arrange(desc(budget))
```

After running the code above, I found that the movie with the highest budget for a movie that is an Animation or Comedy with at least 100 votes was Wild Wild West with a budget of $170,000,000.
